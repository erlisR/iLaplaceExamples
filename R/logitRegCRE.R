...
##' @description  Log-likelihood functions and related quantities for a logistic regression model with crossed random effects based on the \code{\link[iLaplaceExamples]{Salamander}} data. In particular, the function \code{nlogH_salam} gives the joint likelihood for the fixed and random parameters, given the data, \code{grad_salam} gives the gradient of \code{nlogH_salam} with respect to the random effects and \code{hess_salam} gives its Hessian matrix.
##'
...
nlogH_salam <- function(randPar, fixPar, data) {
  ...
  if (length(data) != 3 | is.list(data) != TRUE) stop("data must be a list with elements y, x and z")
    .Call('iLaplaceExamples_nlogH', PACKAGE = 'iLaplaceExamples', randPar, fixPar, data)
}
...
grad_salam <- function(randPar, fixPar, data) {
  ...
  if (length(fixPar) < 6) stop("Fixed parameters must be 6-dimensional vector")
  if (length(data) != 3 | is.list(data) != TRUE) stop("data must be a list with elements y, x and z")
    .Call('iLaplaceExamples_grad_nlogH', PACKAGE = 'iLaplaceExamples', randPar, fixPar, data)
}
...
hess_salam <- function(randPar, fixPar, data) {
  ...
  if (length(fixPar) < 6) stop("Fixed parameters must be 6-dimensional vector")
  if (length(data) != 3 | is.list(data) != TRUE) stop("data must be a list with elements y, x and z")
    .Call('iLaplaceExamples_hess_nlogH', PACKAGE = 'iLaplaceExamples', randPar, fixPar, data)
}
